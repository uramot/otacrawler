...
class TonariNoYJTest < Minitest::Test
  def setup
    ...
    @url = "http://www.tonarinoyj.jp/manga/nekogurui/"
  ...
  def test_url_list
    assert_equal @comic.url, @yj.url_list.last
  end

  def test_title
    assert_equal @comic.title, @yj.titles.last
  end

  def test_authors
    assert_equal @comic.authors.first.name, @yj.authors.last.first
  end

  def test_image_url
    image_url = "http://www.tonarinoyj.jp/manga/assets/assets_c/2015/04/onepunchman-tn-thumb-180xauto-239.jpg"
    assert_equal @comic.image_url, @yj.image_url_list.last
  end
